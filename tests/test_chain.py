...
class TestChainSteps:
    ...
    def sample_document(self):
        ...
        sections = [
            DocumentSection(title="Abstract", content="This paper presents a novel approach."),
            DocumentSection(title="Introduction", content="Background information here."),
    ...
    def test_parse_sections_step_success(self, sample_document):
        ...
        with patch("docxtract.chain.SectionParser") as mock_parser_class:
            ...
            parsed_document = ExtractedDocument(
                title="Parsed Paper",
                sections=[DocumentSection(title="Parsed Section", content="Parsed content")],
                source_file="test.pdf"
            )
    ...
    def test_summarize_sections_step_success(self, sample_document):
        ...
        with patch("docxtract.chain.ChineseSummarizer") as mock_summarizer_class:
            ...
            mock_summarizer.summarize_all_sections.assert_called_once_with(sample_document)

    def test_summarize_sections_step_failure(self, sample_document):
        """Test section summarization step with failure."""
        with patch("docxtract.chain.ChineseSummarizer") as mock_summarizer_class:
            mock_summarizer = Mock()
            mock_summarizer.summarize_all_sections.side_effect = Exception("Summarization failed")
            mock_summarizer_class.return_value = mock_summarizer
    ...
    def test_summarize_overall_step_failure(self, sample_document):
        ...
        with patch("docxtract.chain.ChineseSummarizer") as mock_summarizer_class:
            mock_summarizer = Mock()
            mock_summarizer.summarize_overall.side_effect = Exception("Overall summarization failed")
            mock_summarizer_class.return_value = mock_summarizer
